...
int main() {
    ...
    printf("Running work-stealing OpenMP with %d threads\n", num_threads);
    multiply_matrices_dynamic_workstealing(num_threads);

    printf("Writing result to matrix_C_openmp_worksteal.txt\n");
    write_matrix_to_file("matrix_C_openmp_worksteal.txt", C);
